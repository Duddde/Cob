...
/**
 * Projection déterministe d'un PORTEFEUILLE : somme des projections de
 * chaque poche. Chaque allocation porte son propre capital initial, son
 * versement mensuel, son CAGR, son dividende et son mode (capitalisant /
 * distribuant).
 *
 * @param {object} p
 * @param {Array<{capital:number, monthly?:number, cagr:number,
 *   dividendYield?:number, reinvest?:boolean}>} p.allocations
 * @param {number} p.years
 * @returns {{total:number[], dividends:number[], perAsset:Array}}
 */
export function projectPortfolio({ allocations, years }) {
  const total = Array(years + 1).fill(0);
  const dividends = Array(years + 1).fill(0);
  const perAsset = allocations.map((a) => projectWithDividends({ ...a, years }));
  for (const r of perAsset) {
    for (let y = 0; y <= years; y++) {
      total[y] += r.total[y];
      dividends[y] += r.dividends[y];
    }
  }
  return { total, dividends, perAsset };
}

/**
 * Monte Carlo de PORTEFEUILLE : chaque poche est simulée avec sa propre
 * dérive et sa propre volatilité, puis les trajectoires sont sommées poche
 * par poche AVANT le calcul des percentiles — la diversification apparaît
 * donc dans la largeur de la bande.
 *
 * Hypothèse simplificatrice : les poches sont simulées indépendamment
 * (corrélations ignorées). Les actifs risqués étant en réalité corrélés
 * entre eux, la fourchette réelle d'un portefeuille 100 % actions/crypto
 * est plus large que celle affichée.
 *
 * @param {object} p
 * @param {Array<{capital:number, monthly?:number, cagr:number, vol:number,
 *   dividendYield?:number, reinvest?:boolean}>} p.allocations
 * @param {number}   p.years
 * @param {number[]} [p.percentiles] Défaut [10, 50, 90].
 * @param {number}   [p.paths]       Défaut 2000.
 * @param {number}   [p.seed]        Défaut 42 (décalée par poche).
 * @returns {{[pct: string]: number[]}}
 */
export function projectPortfolioPercentiles({
  allocations,
  years,
  percentiles = [10, 50, 90],
  paths = 2000,
  ...
  allocations.forEach((a, idx) => {
    const dividendYield = a.dividendYield || 0;
    const distributes = a.reinvest === false && dividendYield > 0;
    const growthCagr = distributes ? priceOnlyCagr(a.cagr, dividendYield) : a.cagr;
    const muM = Math.log(1 + growthCagr) / 12;
    const dm = distributes ? monthlyRate(dividendYield) : 0;
    const sigmaM = a.vol / Math.sqrt(12);
    const monthly = a.monthly || 0;
    const normal = makeNormal(mulberry32(seed + idx * 7919));

    for (let p = 0; p < paths; p++) {
      let v = a.capital;
      let cash = 0;
      yearly[0][p] += a.capital;
      for (let m = 1; m <= months; m++) {
        const r = Math.exp(muM + sigmaM * normal()) - 1;
        v = (v + monthly) * (1 + r);
        if (distributes) cash += v * dm;
  ...
  for (const pct of percentiles) out[`p${pct}`] = [];
  for (let y = 0; y <= years; y++) {
    const sorted = Array.from(yearly[y]).sort((a, b) => a - b);
    for (const pct of percentiles) {
      const idx = Math.min(
        sorted.length - 1,
        Math.max(0, Math.round((pct / 100) * (sorted.length - 1)))
